...
const setDatosComplementarios = (data) => {
     axios.post(
          'http://localhost:8000/api/estudiantes/registrar-datos-postulante',
          data
     )
}

const getSedes = () => {
     axios.get('http://localhost:8000/api/estudiantes/get-sedes').then(
          (resp) => {
...
const getProvinciaForSelect = (departamento) => {
     axios.get(
          `http://localhost:8000/api/estudiantes/get-provincias?departamento=${departamento}`
     ).then((resp) => {
          // Llenar las opciones dinámicamente
          resp.data.forEach(function (data) {
               var option = document.createElement('option')
...
const getDistritoForSelect = (departamento, provincia) => {
     axios.get(
          `http://localhost:8000/api/estudiantes/get-distritos?departamento=${departamento}&provincia=${provincia}`
     ).then((resp) => {
          resp.data.forEach(function (data) {
               var option = document.createElement('option')
...
const getUbigeo = (departamento, provincia, distrito) => {
     axios.get(
          `http://localhost:8000/api/estudiantes/get-ubigeo?departamento=${departamento}&provincia=${provincia}&distrito=${distrito}`
     ).then((resp) => {
